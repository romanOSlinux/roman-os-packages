...
## 2026-05-18 (session end — roman-os-iso-next TODO housekeeping)
...
## 2026-05-18 (session end — roman-os-iso-next audit)
...
Plymouth needs BOTH `quiet` and `splash` (not just `splash`) or it falls back to the text details theme. systemd-boot entries live in `/boot/loader/entries/` on standard Arch but `/boot/efi/loader/entries/` on roman-os — and three other path variants on other setups. The wiki's "Configuration" section for any system tool lists exact paths and all variants; skipping it leads to code that works on one machine and silently does nothing on others.
...
Running ATT on bare Arch + XFCE4 surfaces bugs that never appear on roman-os: missing packages that ATT assumes are installed, `fn.distr` returning "arch" where code expects "roman-os", and XFCE-specific wallpaper paths. This is a cheap, high-signal test that catches guard omissions before users hit them. Keep a VirtualBox snapshot at "clean install" state so you can always reset and retest.
...
## 2026-05-18 (session end roman-os-iso build script standardization)
...
## 2026-05-18 (session end roman-os-iso cleanup)
...
## 2026-05-19 (session end roman-os-calamares-config-next promotion)
...
In a project with parallel stable/beta tracks (e.g. `roman-os-calamares-config` + `roman-os-iso`, `roman-os-calamares-config-next` + `roman-os-iso-next`), always trigger the ISO build in the repo that matches the config repo you just pushed to. The ISO build pulls the Calamares package from GitHub Pages, which was published by the config repo's CI. Crossing them (building `roman-os-iso` after pushing to `roman-os-calamares-config-next`) results in the wrong Calamares package being bundled and a confusing mismatch between what was tested and what ships.
